...
terminate(Reason, State = #{socket := Socket}) ->
  send(pg_proto:encode_terminate_msg(), State),
...
begin_startup(State = #{options := Options}) ->
  ...
  send(pg_proto:encode_startup_msg(3, 0, Parameters), State),
  {ok, State}.
  ...
  case recv_msg(State) of
    ...
    authentication_cleartext_password ->
      case Password of
        ...
        _ ->
          send(pg_proto:encode_password_msg(Password), State),
          authenticate(State)
      end;
    authentication_gss ->
    ...
    {authentication_md5_password, Salt} ->
      case Password of
        undefined ->
          {error, missing_password};
        _ ->
          Hash = pg_auth:hash_password_md5(User, Password, Salt),
          send(pg_proto:encode_password_msg(Hash), State),
          authenticate(State)
      end;
...
-spec send_simple_query(Query :: iodata(), state()) ->
        {ok, pg_proto:query_response()} | {error, term()}.
send_simple_query(Query, State) ->
  send(pg_proto:encode_query_msg(Query), State),
  recv_simple_query_response(State, pg_proto:query_response()).

-spec recv_simple_query_response(state(), pg_proto:query_response()) ->
...
recv_simple_query_response(State, Response) ->
  case recv_msg(State) of
    ...
    no_data ->
      ...
      recv_simple_query_response(State, Response2);
    {data_row, Row} ->
      Response2 = pg_proto:add_query_response_row(Row, Response),
      recv_simple_query_response(State, Response2);
    ...
    {ready_for_query, TransactionStatus} ->
      ...
      case maps:find(error, Response) of
        {ok, Error} ->
          {error, Error};
        error ->
          {ok, pg_proto:finalize_query_response(Response2)}
      end;
    Msg ->
      {error, {unexpected_msg, Msg}}
  end.
...
send_extended_query(Query, Params, QueryOptions, State) ->
  ...
  send([pg_proto:encode_parse_msg(<<>>, Query, ParamTypeOids),
        ...
        pg_proto:encode_execute_msg(<<>>, 0),
        pg_proto:encode_sync_msg()],
       State),
...
-spec recv_extended_query_response(state(), pg_proto:query_response()) ->
        {ok, Response :: map()} | {error, term()}.
...
-spec log_backend_notice(pg_proto:error_and_notice_fields()) -> ok.
log_backend_notice(Fields) ->
  #{code := Code, message := Message} = Fields,
  ?LOG_NOTICE("backend notice ~s: ~s", [Code, Message]),
  ok.

-spec log_backend_error(pg_proto:error_and_notice_fields()) -> ok.
