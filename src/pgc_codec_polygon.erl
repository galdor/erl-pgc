...
-module(pgc_codec_polygon).

-behaviour(pgc_codec).

-export([encode/4, decode/4]).

-spec encode(pgc:polygon(), pgc_types:type(), pgc_types:type_set(), []) -> iodata().
...
-spec decode(binary(), pgc_types:type(), pgc_types:type_set(), []) -> pgc:polygon().
