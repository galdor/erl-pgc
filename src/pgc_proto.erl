...
-type msg() :: authentication_ok
             ...
             | bind_complete
             | {command_complete, pgc_proto:command_tag()}
             | {data_row, pgc_proto:row()}
             ...
             | {row_description, [pgc_proto:column()]}.
...
-type query_response() :: #{transaction_status => transaction_status(),
                            columns => [column()],
                            rows => [row()],
                            command_tag => pgc_proto:command_tag(),
...
decode_error_and_notice_field_data($C, Value) ->
  {code, pgc_error_codes:decode(Value)};
